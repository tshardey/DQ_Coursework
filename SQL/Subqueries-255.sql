...
SELECT CAST(COUNT(*) as float) / (SELECT CAST(COUNT(*) as float) from recent_grads) as proportion_abv_avg from recent_grads WHERE ShareWomen > (SELECT AVG(ShareWomen) from recent_grads)
