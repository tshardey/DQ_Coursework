...
def sentence_by_year(df, year):
    df_year = df[df['year'] == year]
    combined_statements = " ".join(df["cleaned_statement"])
    tokens = combined_statements.split(" ")
    tokens_filtered = [t for t in tokens if len(t) > 4]
    token_counts = Counter(tokens_filtered)
    return token_counts.most_common(2)

common_2000= sentence_by_year(sentences_cia, "2000")
common_2002= sentence_by_year(sentences_cia, "2002")
common_2013= sentence_by_year(sentences_cia, "2013")